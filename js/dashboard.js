// dashboard.js — Life OS Dashboard v1
...
function getScheduleLabel(r) {
  if (r.frequency === 'daily') return 'Cada día';
  if (r.frequency === 'weekly') {
    const dow = r.day_of_week ? (DOW_LABELS[r.day_of_week] || r.day_of_week) : '?';
    return `Cada ${dow}`;
  }
  if (r.frequency === 'monthly') return `Día ${r.day_of_month || '?'} de cada mes`;
  return r.frequency;
}
...
async function load() {
  ...
  if (subtitle) {
    ...
    subtitle.textContent = total === 0
      ...
      : `${total} reporte${total !== 1 ? 's' : ''} · ${dueCount > 0 ? dueCount + ' pendiente' + (dueCount > 1 ? 's' : '') : 'al día'}`;
  }
}
...
function reportCardHtml(r) {
  ...
  let dueBadge = '';
  if (!r.response) {
    dueBadge = `<span class="sr-due-badge">⏳ Sin respuesta</span>`;
  } else if (due) {
    dueBadge = `<span class="sr-due-badge overdue">🔄 Actualización pendiente</span>`;
  } else {
  ...
  const responseSection = r.response
    ...
    : `<div class="sr-response-empty">Sin respuesta aún — Claude generará este reporte la próxima vez que trabajes en LifeOS.</div>`;
...
window.showAddReport = function(freq) {
  ...
  document.getElementById('sr-modal-title').textContent = '📊 Nuevo Smart Report';
...
window.editSrReport = async function(id) {
  ...
  document.getElementById('sr-modal-title').textContent = '✏️ Editar Smart Report';
...
window.submitSrReport = async function() {
  ...
  if (_srFreq === 'weekly' && !_srDow) { alert('Selecciona el día de la semana'); return; }
  ...
  if (_srEditId) {
    ...
    // next_run_date = null means it's always due → Claude will generate it on next session
...
window.deleteSrReport = async function(id) {
  if (!confirm('¿Eliminar este Smart Report?')) return;
